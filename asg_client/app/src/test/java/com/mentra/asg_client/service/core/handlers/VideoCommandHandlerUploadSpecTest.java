...
public class VideoCommandHandlerUploadSpecTest {
    ...
    @Test
    public void stop_withS3MultipartUpload_routesToDescribedStopWithMultipartSpec()
            throws Exception {
        JSONObject upload =
                new JSONObject()
                        ...
                        .put(
                                "fields",
                                new JSONObject()
                                        .put("recordingId", "rec-1")
                                        .put("authToken", "tok-abc"));
        JSONObject data = new JSONObject().put("requestId", "req-mp").put("upload", upload);

        boolean handled = handler.handleStopCommand(data);

        assertThat(handled).isTrue();
        verify(streamingManager).sendStopRecordingAck("req-mp");
        // The handler forwards the parsed spec; MediaCaptureService picks the multipart executor.
        ArgumentCaptor<UploadSpec> specCaptor = ArgumentCaptor.forClass(UploadSpec.class);
        verify(captureService).handleStopVideoCommand(eq("req-mp"), specCaptor.capture());
        assertThat(specCaptor.getValue().multipart).isNotNull();
        assertThat(specCaptor.getValue().multipart.recordingId).isEqualTo("rec-1");
        assertThat(specCaptor.getValue().upload).isNull();
    }
