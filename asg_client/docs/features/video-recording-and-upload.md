...
### Resumable (S3 multipart) upload (`body: "s3_multipart"`)

For long clips on flaky Wi-Fi, the `upload` descriptor can bootstrap a **glasses-driven S3 multipart** upload instead of a single request — it **takes precedence** over the single described upload when present. The `upload` carries `body: "s3_multipart"`, the app's API base in `url`, and `recordingId` + `authToken` in `fields`.

The glasses drive the upload against the app's API (all `POST`, bearer-authed; `base` = `<url>/recordings/<recordingId>/segments/multipart`):

1. **initiate** — `POST {base}` → `{segment_index, upload_id, part_size}`.
2. **part URLs** — `POST {base}/part-urls` with the part numbers → `{urls: {"<n>": "<presigned PUT>"}}`.
3. **upload parts** — `PUT` each part's bytes to its presigned URL, collecting the `ETag` per part.
4. **complete** — `POST {base}/complete` with the `{part_number, etag}` list.
5. **abort** — on terminal failure, `POST {base}/abort` so S3 drops the orphaned parts.

Why it exists: a dropped Wi-Fi connection only re-sends the **in-flight part**, not the whole clip. Each part is retried independently (transient/`5xx`, up to 6 attempts with capped backoff 2s→6s) to outlast brief DNS blips (`Unable to resolve host …s3.amazonaws.com`). There is **no overall call deadline** (a multi-minute clip on a slow link isn't aborted), only per-part write/read timeouts. Progress is reported at **part granularity** through the same `uploading` status.
...
| `uploading`        | `progress`, `bytesSent`, `bytesTotal`    | Progress, throttled (~every 5% + final 100%). For `s3_multipart`, reported per part. |
...
- `upload_video` carries the same `upload` / `onComplete` descriptor (including the `s3_multipart` variant) and the original `requestId`.
- The glasses locate the clip by `requestId` — recordings live at `<mediaDir>/VID_<timestamp>_<rand>_<requestId>/base.mp4`, so the capture directory name ends with the `requestId` — and run the described (or `s3_multipart`) upload again. When a start-retry left several dirs with the same `requestId` (aborted 0-byte attempts plus the real recording), the glasses pick the **largest** `base.mp4` so a re-upload never grabs an empty/abandoned clip.
